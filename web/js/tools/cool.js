...
Q.Tool.define("EngageUsers/cool", function (options) {
	var tool = this;
	...
	Q.addStylesheet("css/EngageUsers.css", p.fill('stylesheet'), { slotName: 'EngageUsers' });
	Q.Text.get('EngageUsers/content', p.fill('text'));
...
{ // methods go here
	...
	refresh: function () {
		...
		tool.getMyStream(function (err, stream) {
			Q.Template.render('EngageUsers/cool/view', fields, function (err, html) {
				// replace all the HTML
				tool.element.innerHTML = html;
			
				// save some elements in an object
				elements.a = tool.element.getElementsByClassName('EngageUsers_foo');
				elements.b = tool.element.getElementsByClassName('EngageUsers_b');
...
Q.Template.set('EngageUsers/cool/view',
	'<div>'
	+	'<p class="EngageUsers_a">{{foo.bar}}</p>'
	+	'<input class="EngageUsers_sum" value="{{baz}}">'
